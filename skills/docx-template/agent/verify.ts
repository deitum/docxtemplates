/**
 * `verify` — prove the template actually works.
 *
 * This is the tool the equivalent docxtemplater skill has no answer for: the
 * skill ships inside the render engine, so a generated template can be listed,
 * rendered with sample data, and read back as text without ever leaving the
 * command line. Nothing is "probably fine"; either the report comes out or the
 * errors say why.
 *
 * Usage:
 *   node verify.mjs <template.docx> [data.json] [-o report.docx]
 *                   [--delimiter +++] [--aliases aliases.json]
 *                   [--context context.mjs] [--allow-nullish] [--quiet]
 */
...
import { type CommandSummary } from '../../../src/types';
import {
  type AliasOptions,
  isEntryPoint,
  parseArgs,
...
/** The loop variable of a `FOR`/`END-FOR` command, i.e. its first token. */
const loopVarOf = (code: string): string => code.trim().split(/\s+/)[0] ?? '';

/**
 * Static checks over the command list. These catch the mistakes that produce
 * the most confusing runtime errors — an unclosed loop swallows the rest of
 * the document, and a smart quote turns a working expression into a syntax
 * error the moment someone edits the template in Word.
 */
export function checkCommands(commands: CommandSummary[]): string[] {
  const problems: string[] = [];
  const stack: { type: 'FOR' | 'IF'; label: string }[] = [];
  ...
  for (const cmd of commands) {
    ...
    if (SMART_QUOTES.test(cmd.code)) {
      problems.push(
        `${cmd.type} ${cmd.code} contains a curly quote. Word autocorrects ` +
          `straight quotes; retype them, or render with fixSmartQuotes: true.`
      );
    }
  }
  ...
  for (const open of stack.reverse()) {
    problems.push(
      open.type === 'FOR'
        ? `FOR ${open.label} is never closed by an END-FOR`
        : `IF ${open.label} is never closed by an END-IF`
    );
  }
...
/** Loads `additionalJsContext` from a module's default export. */
async function loadContext(file: string): Promise<object> {
  const mod = (await import(pathToFileURL(requireFile(file)).href)) as {
    default?: unknown;
  };
...
async function main(): Promise<void> {
  ...
  if (templateArg == null) {
    console.error(
      'Usage: node verify.mjs <template.docx> [data.json] [-o report.docx] ' +
        '[--delimiter +++] [--aliases aliases.json] [--context context.mjs] ' +
        '[--allow-nullish] [--quiet]'
    );
    process.exit(1);
  }
  ...
  const aliases: AliasOptions = readAliases(flags);

  // 1. What does the engine see in this template?
  ...
  const outputPath = path.resolve(
    typeof outFlag === 'string'
      ? outFlag
      : templatePath.replace(/\.docx$/i, '_report.docx')
  );
  ...
  const report = await createReport({
    ...
    ...(typeof contextFlag === 'string'
      ? { additionalJsContext: await loadContext(contextFlag) }
      : {}),
    ...aliases,
  });
  ...
  // 3. What came out? Reading it back is the only way to catch a template that
  //    renders cleanly but says the wrong thing.
